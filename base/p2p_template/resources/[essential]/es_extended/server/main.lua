...
if not Config.OxInventory then
	...
	function getItemVariation(itemname, count)
		if Config.PickupsList[itemname].var == true then
			for i = 1, #Config.PickupsList[itemname].variations do
				if count >= Config.PickupsList[itemname].variations[i].count and count <= Config.PickupsList[itemname].variations[i].count2 then
				return Config.PickupsList[itemname].variations[i].varName
				end
			end
			return Config.PickupsList[itemname].name
		else
			return Config.PickupsList[itemname].name
		end
	end
		...
		if itemType == 'item_standard' then
			if itemCount == nil or itemCount < 1 then
				...
				if (itemCount > xItem.count or xItem.count < 1) then
					...
					if Config.PickupsList[itemName] == nil then
						ESX.CreatePickup('item_standard', itemName, itemCount, pickupLabel, playerId, "v_serv_abox_02")
					else
						pickItem = getItemVariation(itemName, itemCount)
						ESX.CreatePickup('item_standard', itemName, itemCount, pickupLabel, playerId, pickItem)
					end
		...
		elseif itemType == 'item_account' then
			if itemCount == nil or itemCount < 1 then
				...
				if (itemCount > account.money or account.money < 1) then
					...
					ESX.CreatePickup('item_account', itemName, itemCount, pickupLabel, playerId, "prop_cash_pile_01")
