...
local function GetAvailableSeat(seatCoords, seats, raycast)
	...
	for _index, data in pairs(sortedSeats) do
		if IsSeatAvailable(data.coords, 'sit') then
			coords = data.coords
			heading = --[[GetEntityHeading(PlayerPedId()) - 180.0 or]] data.heading
			break
		end
	end
		...
		if data.enabled then
			for name, info in pairs(data.polys) do
				...
				if info.poly == nil then
					print("^Erreur: PolyZone '"..name.."' n'a pas pu être généré! (manque de spécifications poly)")
				elseif info.lay == nil and info.sit == nil then
					print("^Erreur: PolyZone '"..name.."' n'a pas pu être généré! (aucune action assignée)")
				else
					local type = 'sit'
					local options = {}
			...
			print("^2Info: PolyZone group '"..group.."' a été généré.")
		else
			print("^3Info: PolyZone group '"..group.."' est désactivé.")
		end
	end
end
