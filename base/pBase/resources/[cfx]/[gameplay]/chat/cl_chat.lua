...
RegisterKeyMapping('togglechat', 'Changer le type ', 'keyboard', 'l')
RegisterKeyMapping('closechatonmap', 'Fermer le chat sur la carte', 'keyboard', 'ESCAPE')
...
RegisterCommand('toggleChat', function()
  ...
  if chatLoaded then
    --local forceHide = IsScreenFadedOut() or IsPauseMenuActive() # -- : "Hidden, Visible and When Active" apparaissent quand la carte est ouverte
...
RegisterCommand('closechatonmap', function()
  forceHide2 = chatHideState == CHAT_HIDE_STATES.ALWAYS_HIDE or true
  chatHideState = CHAT_HIDE_STATES.ALWAYS_HIDE
  SendNUIMessage({
    type = 'ON_SCREEN_STATE_CHANGE',
    hideState = chatHideState,
    fromUserInteraction = not forceHide2 and not isFirstHide and not wasForceHide
  })
end, false)