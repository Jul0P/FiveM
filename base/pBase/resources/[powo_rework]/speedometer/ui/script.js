$(document).ready(function(){
  ...
  window.addEventListener("message", function(event){
    ...
    if(event.data.showhud == true){
      ...
      if(event.data.speed >= 50){
        $('.progress-speed').removeClass('orangeStroke');
        $('.progress-speed').addClass('redStroke');
      }
      if(event.data.speed >= 30){
        $('.progress-speed').addClass('orangeStroke');
      }
    }
  ...
  function setProgressFuel(percent, element){
    ...
    const offset = circumference - ((-percent*63)/100) / 100 * circumference;
  ...
  function setProgressSpeed(value, element){
    ...
    const offset = circumference - ((-percent*35)/100) / 100 * circumference;
    circle.style.strokeDashoffset = -offset;
