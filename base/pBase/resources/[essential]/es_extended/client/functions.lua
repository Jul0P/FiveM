...
function ESX.ShowNotification(message, timeout, position, progress, theme, exitAnim, flash, pin_id)
    while IsPauseMenuActive() do -- source de bug
        Citizen.Wait(100)
    end
    -- if IsPauseMenuActive() then return end

    exports.notif:Send(message, timeout, position, progress, theme, exitAnim, flash, pin_id)
end
...
function ESX.ShowAdvancedNotification(title, subject, message, icon, timeout, position, progress, theme, exitAnim, flash, pin_id)
    while IsPauseMenuActive() do -- source de bug
        Citizen.Wait(100)
    end
    -- if IsPauseMenuActive() then return end

    exports.notif:SendAdvanced(message, title, subject, icon, timeout, position, progress, theme, exitAnim, flash, pin_id)
end
...
function ESX.UI.ShowInventoryItemNotification(add, item, count)
    local prepText = tostring(count) .. ' ' .. tostring(item)
...
function ESX.Game.Utils.DrawText2D(text, x, y, size)
...
AddEventHandler('esx:showNotification', function(message, timeout, position, progress, theme, exitAnim, flash, pin_id)
    ESX.ShowNotification(message, timeout, position, progress, theme, exitAnim, flash, pin_id)
end)

RegisterNetEvent('esx:showAdvancedNotification')
AddEventHandler('esx:showAdvancedNotification', function(title, subject, message, icon, timeout, position, progress, theme, exitAnim, flash, pin_id)
    ESX.ShowAdvancedNotification(title, subject, message, icon, timeout, position, progress, theme, exitAnim, flash, pin_id)
end)
