if Config.ActivateTakeOtage.Activate then
    ...
    RegisterCommand(Config.ActivateTakeOtage.Command,function()
    ...
    function takeHostage()
        ...
        if closestPlayer ~= nil then
            ...
            holdingHostage = true
            TriggerServerEvent('pBase:anim:sync', closestPlayer, lib, lib2, anim1, anim2, distans, distans2, height, target, length, spin, controlFlagMe, controlFlagTarget, animFlagTarget, attachFlag)
        else
            ESX.ShowNotification("Aucun joueur à proximité")
        end
    end

    RegisterNetEvent('pBase:anim:syncTarget')
    AddEventHandler('pBase:anim:syncTarget', function(target, animationLib, animation2, distans, distans2, height, length,spin,controlFlag,animFlagTarget,attach)
    ...
    RegisterNetEvent('pBase:anim:syncMe')
    AddEventHandler('pBase:anim:syncMe', function(animationLib, animation,length,controlFlag,animFlag)
        local playerPed = PlayerPedId()
    ...
    RegisterNetEvent('pBase:anim:cl_stop')
    AddEventHandler('pBase:anim:cl_stop', function()
        holdingHostageInProgress = false
        beingHeldHostage = false
        ...
        while true do
            if holdingHostage then
                if IsEntityDead(PlayerPedId()) then
                    ...
                    TriggerServerEvent("pBase:anim:stop",target)
                    Wait(100)
                    releaseHostage()
                end
