if Config.ActivateSlashtires then
				...
				if CanUseWeapon(allowedWeapons) then
					...
					if closestTire ~= nil then
						if IsVehicleTyreBurst(vehicle, closestTire.tireIndex, 0) == false then
							attente = 1
							Draw3DText(closestTire.bonePos.x, closestTire.bonePos.y, closestTire.bonePos.z, tostring("[~r~E~s~] Crever le pneu"))
							if IsControlJustPressed(1, 38) then
								...
								if driverServer == 0 then
									...
									TriggerServerEvent("pBase:TargetClient", driverServer, closestTire.tireIndex)
								end
								Citizen.Wait((animDuration / 2) * 1000)
	...
	RegisterNetEvent("pBase:SlashClientTire")
	AddEventHandler("pBase:SlashClientTire", function(tireIndex)
