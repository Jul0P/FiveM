if Config.ActivateHideTrunk then
	local inTrunk = false
		...
		while true do
			if inTrunk then
				...
				if DoesEntityExist(vehicle) and not IsPedDeadOrDying(playerPed, true) then
					...
					ESX.ShowHelpNotification('~INPUT_TALK~ Sortir du coffre')
					DisplayRadar(false)
		...
		while true do
			if trunkCoords then
				...
				if displayTrunkOpen then
					ESX.Game.Utils.DrawText3D(trunkCoords, '[~b~E~s~] Se cacher\n[~b~H~s~] Ouvrir\n[~b~L~s~] Coffre', 0.8)
				else
					ESX.Game.Utils.DrawText3D(trunkCoords, '[~b~E~s~] Se cacher\n[~b~H~s~] Fermer\n[~b~L~s~] Coffre', 0.8)
				end
			else
			...
			if not IsPedInAnyVehicle(playerPed, true) then
				...
				if DoesEntityExist(vehicle) and distance < 10 then
					...
					if trunk ~= -1 then
						...
						if #(GetEntityCoords(playerPed) - coords) <= 1.3 then
							attente = 5
							...
							if isAPlayer and not inTrunk then
								if GetVehicleDoorAngleRatio(vehicle, 5) < 0.8 then
									trunkCoords = coords
