	...
	if Config.DisableWeaponWheel or Config.DisableAimAssist or Config.DisableVehicleRewards then
		CreateThread(function()
			while true do
				...
				if Config.DisableRoulade then
					if IsPlayerFreeAiming(PlayerId()) then 
						DisableControlAction(0, 22, true)
					elseif IsPedArmed(PlayerPedId(), 6) then
						DisableControlAction(1, 26, true)
					end
				end

				if Config.DisableCoupdecrosse then
				...
				if Config.SetPNJAmount.Activate then
					SetVehicleDensityMultiplierThisFrame(Config.SetPNJAmount.SetVehicleDensityMultiplierThisFrame)
					SetPedDensityMultiplierThisFrame(Config.SetPNJAmount.SetPedDensityMultiplierThisFrame)
					SetRandomVehicleDensityMultiplierThisFrame(Config.SetPNJAmount.SetRandomVehicleDensityMultiplierThisFrame)
					SetParkedVehicleDensityMultiplierThisFrame(Config.SetPNJAmount.SetParkedVehicleDensityMultiplierThisFrame)
					SetScenarioPedDensityMultiplierThisFrame(Config.SetPNJAmount.SetScenarioPedDensityMultiplierThisFrame, Config.SetPNJAmount.SetScenarioPedDensityMultiplierThisFrame)
				end
...
if not Config.OxInventory then
	...
	AddEventHandler('esx:createPickup', function(pickupId, label, coords, itemType, name, components, tintIndex)
		...
		if itemType == 'item_weapon' then
			...
			ESX.Game.SpawnLocalObject(components, coords, setObjectProperties)
		end
	end)
