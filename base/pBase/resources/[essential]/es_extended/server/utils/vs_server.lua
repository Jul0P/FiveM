...
RegisterCommand('freezetime', function(source, args)
    if source ~= 0 then
        if isAllowedToChange(source) then
            ...
            if freezeTime then
                TriggerClientEvent('esx:showNotification', source, 'Temps est ~b~freeze~s~.')
            else
                TriggerClientEvent('esx:showNotification', source, 'Temps n\'est plus ~y~freeze~s~.')
            end
        else
    ...
    if source ~= 0 then
        if isAllowedToChange(source) then
            ...
            if not DynamicWeather then
                TriggerClientEvent('esx:showNotification', source, 'Changements métérologiques dynamiques ~r~desactivé~s~.')
            else
                TriggerClientEvent('esx:showNotification', source, 'Changements métérologiques dynamiques ~b~activé~s~.')
            end
        else
            TriggerClientEvent('chatMessage', source, '', {255,255,255}, '^8Error: ^1Vous n\'etes pas autorisé a utiliser cette commande.')
        end
    ...
    if source == 0 then
        ...
        if isAllowedToChange(source) then
            ...
            if args[1] == nil then
                ...
                if validWeatherType then
                    TriggerClientEvent("esx:showNotification", source, 'Météo vient de changer pour : ~y~' .. string.lower(args[1]) .. "~s~.")
                    CurrentWeather = string.upper(args[1])
                    newWeatherTimer = 10
                    TriggerEvent('vSync:requestSync')
                else
...
RegisterCommand('blackout', function(source)
    if source == 0 then
        ...
        if isAllowedToChange(source) then
            blackout = not blackout
            if blackout then
                TriggerClientEvent("esx:showNotification", source, 'Blackout est ~r~activé~s~.')
            else
                TriggerClientEvent("esx:showNotification", source, 'Blackout est ~r~désactivé~s~.')
            end
            TriggerEvent('vSync:requestSync')
        end
    end
...
RegisterCommand('morning', function(source)
    ...
    if isAllowedToChange(source) then
        ...
        TriggerClientEvent("esx:showNotification", source, 'Heure réglée sur ~y~matin~s~.')
        TriggerEvent('vSync:requestSync')
    end
end)
RegisterCommand('noon', function(source)
    ...
    if isAllowedToChange(source) then
        ShiftToMinute(0)
        ShiftToHour(12)
        TriggerClientEvent("esx:showNotification", source, 'Heure réglée sur ~y~nuit~s~.')
        TriggerEvent('vSync:requestSync')
    end
end)
RegisterCommand('evening', function(source)
    ...
    if isAllowedToChange(source) then
        ShiftToMinute(0)
        ShiftToHour(18)
        TriggerClientEvent("esx:showNotification", source, 'Heure réglée sur ~y~soir~s~.')
        TriggerEvent('vSync:requestSync')
    end
end)
    ...
    elseif source ~= 0 then
        if isAllowedToChange(source) then
            if tonumber(args[1]) ~= nil and tonumber(args[2]) ~= nil then
                ...
                TriggerClientEvent('esx:showNotification', source, 'Heure réglée sur : ~y~' .. newtime .. "~s~!")
                TriggerEvent('vSync:requestSync')
            else
