...
function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, job2, loadout, name, coords, metadata)
	...
	function self.showAdvancedNotification(title, subject, message, icon, timeout, position, progress, theme, exitAnim, flash, pin_id)
		self.triggerEvent('esx:showAdvancedNotification', title, subject, message, icon, timeout, position, progress, theme, exitAnim, flash, pin_id)
	end
